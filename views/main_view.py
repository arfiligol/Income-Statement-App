...
class MainView(ttk.Window):
    ...
    def create_navigation_bar(self) -> None:
        ...
        navigation_bar.pack(side=TOP, fill=X, pady=0, expand=False)  # expand = NO, make sure it won't expand on the vertical axis
    ...
    def create_target_file_frame(self) -> None:
        ...
        select_file_btn = ttk.Button(
            target_file_container,
            text="選擇檔案",
            command=lambda: self.select_source_file(select_file_text),
            bootstyle="primary",
        )
    ...
    def create_output_config_frame(self) -> None:
        ...
        selected_dir_label = ttk.Label(
            dir_frame,
            textvariable=selected_dir_text,
            font=("Microsoft Yahei", 14),
            anchor=W,
            bootstyle="white",
        )
        ...
        dir_button = ttk.Button(
            dir_frame,
            text="選擇資料夾",
            command=lambda: self.select_output_dir(selected_dir_text),
            bootstyle="primary",
        )
    ...
    def select_source_file(self, hint_text: ttk.StringVar) -> None:
        selected_filename = self.controller.open_excel_file()
        hint_text.set(selected_filename or "")

    # When user select a output folder
    def select_output_dir(self, hint_text: ttk.StringVar) -> None:
        output_dir = self.controller.choose_output_dir()
        hint_text.set(output_dir or "")
